...
test('Graph tab uses Primer icon and text component slots', () => {
  ...
  assert.match(source, /icon\.setAttribute\('data-component', 'Octicon'\)/);
...
test('Graph icon keeps the native Octicon presentation attributes', () => {
  assert.match(source, /icon\.setAttribute\('fill', 'currentColor'\)/);
  assert.match(source, /icon\.setAttribute\('focusable', 'false'\)/);
  assert.match(source, /icon\.setAttribute\('style', 'vertical-align:text-bottom'\)/);
});
